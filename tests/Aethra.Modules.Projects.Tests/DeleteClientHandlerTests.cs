...
public sealed class DeleteClientHandlerTests(ProjectsDbFixture fixture)
    : IClassFixture<ProjectsDbFixture>
{
    private static readonly DateTimeOffset Now = new(2026, 1, 1, 12, 0, 0, TimeSpan.Zero);

    [Fact]
    public async Task Handle_forced_delete_enqueues_instance_cleanup_before_removing_client_graph()
    {
        await using var db = await fixture.CreateCleanDbContextAsync();
        var (client, instance) = SeedClientGraph(db);
        var outbox = new RecordingOutbox();
        var handler = new DeleteClientHandler(db, new FixedClock(Now), outbox);

        var result = await handler.Handle(new DeleteClientCommand(client.Id.ToString(), Force: true),
            CancellationToken.None);

        result.IsSuccess.Should().BeTrue();
        (await db.Clients.AnyAsync(c => c.Id == client.Id)).Should().BeFalse();
    ...
    [Fact]
    public async Task Handle_rejects_client_with_instances_without_force_before_cleanup_or_mutation()
    {
        await using var db = await fixture.CreateCleanDbContextAsync();
        var (client, instance) = SeedClientGraph(db);
        var outbox = new RecordingOutbox();
        ...
        var result = await handler.Handle(new DeleteClientCommand(client.Id.ToString(), Force: false),
            CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("client.has_instances");
        outbox.Events.Should().BeEmpty();
        (await db.Clients.AnyAsync(c => c.Id == client.Id)).Should().BeTrue();
        (await db.Instances.AnyAsync(i => i.Id == instance.Id)).Should().BeTrue();
    }

    private static (Client Client, Instance Instance) SeedClientGraph(ProjectsDbContext db)
    {
