...
/// <summary>
/// Construye la lista de env vars a inyectar en una Application cuando se crea/rota un binding.
/// Cada <see cref="ServiceType"/> tiene su propio conjunto canónico (DATABASE_URL, REDIS_URL, etc.).
/// El prefix opcional permite que una misma app bindée dos servicios del mismo tipo
...
public sealed class DefaultBindingEnvVarMapper : IBindingEnvVarMapper
{
    ...
    [
        Var(prefix, "DATABASE_URL", $"Host={host};Port={port};Database={db};Username={creds.Username};Password={creds.Password};", secret: true),
        ...
        Var(prefix, "POSTGRES_PASSWORD", creds.Password, secret: true),
    ];

    private static List<EnvVarUpsert> BuildRedis(string prefix, string host, int port, string keyspace, BindingCredentials creds) =>
    [
        Var(prefix, "REDIS_URL", $"redis://{creds.Username}:{creds.Password}@{host}:{port}/0", secret: true),
        ...
        Var(prefix, "REDIS_PASSWORD", creds.Password, secret: true),
    ...
    [
        Var(prefix, "RABBITMQ_URL", $"amqp://{creds.Username}:{creds.Password}@{host}:{port}/{vhost}", secret: true),
        ...
        Var(prefix, "RABBITMQ_PASSWORD", creds.Password, secret: true),
    ];

    private static List<EnvVarUpsert> BuildMySQL(string prefix, string host, int port, string db, BindingCredentials creds) =>
    [
        Var(prefix, "MYSQL_URL", $"Server={host};Port={port};Database={db};Uid={creds.Username};Pwd={creds.Password};", secret: true),
        ...
        Var(prefix, "MYSQL_PASSWORD", creds.Password, secret: true),
    ];

    private static List<EnvVarUpsert> BuildMongoDB(string prefix, string host, int port, string db, BindingCredentials creds) =>
    [
        Var(prefix, "MONGODB_URL", $"mongodb://{creds.Username}:{creds.Password}@{host}:{port}/{db}", secret: true),
        ...
        Var(prefix, "MONGODB_PASSWORD", creds.Password, secret: true),
    ];

    private static EnvVarUpsert Var(string prefix, string key, string value, bool secret = false)
        => new($"{prefix}{key}", value, IsBuildTime: false, IsRuntime: true, IsSecret: secret);
}
